...
const AdvModal = ({ isOpen, onClose, id, modelSrc, heading, modalText }) => {
  ...
  return ReactDOM.createPortal(
    <div className="modal-backdrop fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center">
      <div className="modal-content bg-slate text-white px-8 py-4 rounded-2xl shadow-lg flex flex-col md:flex-row gap-4 items-center relative w-4/5 h-auto overflow-y-scroll overflow animate-slide-up">
        <button onClick={onClose} className="absolute top-6 right-6">
          <img src={cross} alt="" className="" />
        </button>
        
        <img src={modelSrc} alt="pic" className="h-2/3 w-2/5" />
